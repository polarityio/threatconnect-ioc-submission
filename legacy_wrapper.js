...
/**
 * WARNING!!!
 * The global context is executed every time a command is sent to this file
 * from the server. This means that any code like this:
    ...
    In the body of the file is inherently unsafe as it will execute with every command
    sent to the nodeJS process. Each command will add another event handler eventually causing
    a warning as the threshold for max number of listeners is exceeded.

    Additional information and context can be found here:
    https://linear.app/polarity/issue/PL-702/error-occuring-on-cev5-server
 */
...
/**
 * maybe_add_uncaught_exception_handler helps ensures that only one event
 * handler is added to the process obj for uncaughtException events. If
 * we don't restrict this behavior infinite event handlers will be added
 * potentially causing memory leaks.
*/
const maybe_add_uncaught_exception_handler = () => {
  // note the negated condition, we only want to execute the if block
  // when process does not have an event handler for uncaught exceptions.
...
maybe_add_uncaught_exception_handler()
